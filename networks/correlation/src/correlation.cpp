...
static void correlate_patch(TensorAccessor<scalar_t, 3> input1,
...
template <typename scalar_t>
static void correlate_patch_grad(TensorAccessor<scalar_t, 3> input1,
...
std::vector<at::Tensor> correlation_cpp_backward(at::Tensor input1,
                                                 at::Tensor input2,
    ...
    AT_DISPATCH_FLOATING_TYPES(input1.type(), "correlation_forward_cpp", ([&] {
                                 ...
                                 for (ph = 0; ph < patchH; ++ph) {
                                   for (pw = 0; pw < patchW; ++pw) {
                                     for (h = 0; h < oH; ++h) {
                                       for (w = 0; w < oW; ++w) {
                                         correlate_patch_grad(input1_acc[n],
                                                              gradInput1_acc[n],
                                                              input2_acc[n],
                                                              gradInput2_acc[n],
